...
const page_1 = __importDefault(require("../component/login/page"));
const page_2 = __importDefault(require("../component/register/page"));
...
function Navbar() {
    ...
    return (<>
      ...
      <button className="font-bold px-4 py-2 rounded-xl hover:bg-gray-700 transition cursor-pointer">
        ...
        {!isLoggedIn ? (<>
            <button onClick={() => setShowLogin(true)} className="font-bold px-4 py-2 rounded-xl hover:bg-gray-700 transition cursor-pointer">
              Log in
            </button>
            <button onClick={() => setShowRegister(true)} className="font-bold px-4 py-2 rounded-xl hover:bg-gray-700 transition cursor-pointer">
              Sign up
            </button>
          </>) : (<div className="relative">
            <button onClick={() => setOpen(!open)} className="w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center">
              <span className="text-white font-bold cursor-pointer">U</span>
            </button>

            {open && (<div className="absolute right-0 mt-2 w-48 bg-white text-black rounded shadow-lg">
                ...
                <button className="block w-full text-left px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={handleLogout}>
                  Logout
                </button>
              </div>)}
          </div>)}
      ...
      {/* ✅ Register Modal */}
      {showRegister && (<Modal onClose={() => setShowRegister(false)}>
          <page_2.default onClose={function () {
...
function ConditionalHero() {
    const pathname = (0, navigation_1.usePathname)();
    if (pathname !== "/")
        return null; // hide on login & dashboard
    return <HeroSection />;
}
// ✅ Hero Section
function HeroSection() {
    return (<div className="relative h-screen w-full bg-cover bg-center flex items-center justify-center" style={{
            backgroundImage: "url('https://cf.bstatic.com/xdata/images/hotel/max1024x768/608206304.jpg?k=589625fdd983e0f631bac11b36947ac46aa1be8c93c7bc610c4f9be327041eeb&o=')",
        }}>
